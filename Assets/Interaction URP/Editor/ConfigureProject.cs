...
namespace InteractionURP.Editor
{
    public static class ConfigureProject
    {
        ...
        private static bool TryFindRendererDataAsset(out UniversalRendererData data)
        {
        ...
        private static void UpdateMainLayerMask(UniversalRendererData data)
        {
            var invMask = ~((1 << 6) | (1 << 7));
            data.opaqueLayerMask  = data.opaqueLayerMask & invMask;
        }

        private static void AddStencilMaskFeature(UniversalRendererData data)
        {
        ...
        private static void AddStencilVisFeature(UniversalRendererData data)
        {
            var renderObj = ScriptableObject.CreateInstance<RenderObjects>();
